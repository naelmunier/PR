...
export interface GradientCardProps {
  gradient?: "orange" | "gray" | "purple" | "green" | "red";
  badgeText: string;
  badgeColor: string;
  title: string;
  description: string;
...
export function GradientCard({
  ...
  const gradientClass = gradientMap[gradient] ?? gradientMap.gray;
  ...
  return (
    <motion.div
      ...
      className="h-full"
    >
      <div className={`relative flex flex-col justify-between h-full w-full overflow-hidden rounded-2xl p-6 shadow-sm transition-shadow duration-300 hover:shadow-lg ${gradientClass} ${className}`}>
        {imageUrl && (
          <motion.img
            src={imageUrl}
            alt=""
